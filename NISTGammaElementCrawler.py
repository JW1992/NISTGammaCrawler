...
def main():
    ...
    result = session_requests.get(URL_BASE+"tab3.html")
    ...
    for link in soup.body.center.table.find_all(href=True):
        ...
        element_name.append(link.string)
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS elements
                     (name text, symbol text, atom_number int)''')
    # Clear table
    c.execute('''DELETE FROM elements''')
    # Insert a row of data
    for i in range(0, 92):
        c.execute("INSERT INTO elements VALUES (?, ?, ?)", (element_name[i].replace(', ', ''), element_symbol[i].replace(' ',''), int(atom_number[i])))
    ...
    for i in range(0,0):
        ...
        # Create table
        c.execute("CREATE TABLE IF NOT EXISTS " + element_name[i].replace(', ', '') +" (absedge text, energy real, attencoeff real)")
        c.execute("DELETE FROM " + element_name[i].replace(', ', ''))

        print("Saving data table for "+element_name[i].replace(', ', ''))

        for row in tagTemp.find_all('tr', )[5:]:
            elements = row.find_all('td', rowspan=False)
            if elements.__len__() == 3:
                #strTemp = "INSERT INTO " + element_name[i] + "  (energy, attencoeff) VALUES (" + float(elements[0].string) + ", " + elements[1] + ")"
                #c.execute(strTemp)
                ...
                c.execute("INSERT INTO {} (absedge, energy, attencoeff) VALUES (?, ?, ?)".format(element_name[i].replace(', ', '')),
